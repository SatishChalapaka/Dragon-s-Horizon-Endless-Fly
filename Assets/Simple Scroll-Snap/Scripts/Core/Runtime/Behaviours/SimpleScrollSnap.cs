...
namespace DanielLochner.Assets.SimpleScrollSnap
{
    ...
    public class SimpleScrollSnap : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        {
            playerCameraShop = GameObject.FindGameObjectWithTag("PlayerCameraCanvasShop").GetComponent<Camera>();
        ...
        private void Start()
        {
            ...
            //GoToPanel(PlayerPrefs.GetInt("SavedPlayerNumber"));
        }
        private void Update()
        {
        ...
        public void OnEndDrag(PointerEventData eventData)
        {
            ...
            switch (movementAxis)
            {
                case MovementAxis.Horizontal:
                    ...
                    ////if (playerCameraShop.transform.position.x == 500) return;
                    ...
                    //if (Velocity.x > 0 || displacementFromCenter.x < 0f)
                    //{
                    //    playerCameraShop.transform.position = Vector3.Lerp(new Vector3(playerCameraShop.transform.position.x, playerCameraShop.transform.position.y, playerCameraShop.transform.position.z),
                    //        new Vector3(playerCameraShop.transform.position.x + 2, playerCameraShop.transform.position.y, playerCameraShop.transform.position.z),1f);
                    //}
                    //if (Velocity.x < 0 || displacementFromCenter.x > 0f)
                    //{
        ...
        public delegate void MovementDeligate();
        public MovementDeligate movementDeligate;
        ...
        public void GoToPanel(int panelNumber)
        {
            ...
            for (int i = 0; i < Panels.Length; i++)
            {
                if (CenteredPanel == i)
                {
                    ...
                    //GameManager.  playerCameraShop.transform.position = new Vector3(playerCameraShop.transform.position.x - 2, playerCameraShop.transform.position.y, playerCameraShop.transform.position.z);
                    //Content.GetChild(CenteredPanel).transform.localScale = new Vector3(1, 1, 1);
                    //Content.GetChild(CenteredPanel).transform.GetChild(2).gameObject.SetActive(true);
                    //movementDeligate();
                }
                else
                {

                    //Content.GetChild(i).transform.localScale = new Vector3(0.7f, 0.7f, 0.7f);
                    //Content.GetChild(i).transform.GetChild(2).gameObject.SetActive(false);
                    //playerCameraShop.transform.position = new Vector3(playerCameraShop.transform.position.x + 2, playerCameraShop.transform.position.y, playerCameraShop.transform.position.z);
                }
            }
        ...
        public void GoToPreviousPanel(int count)
        {
            int nearestPanel = GetNearestPanel();
            if (nearestPanel != 0)
            {
                GoToPanel(nearestPanel - 1);
                playerCameraShop.transform.position = new Vector3(playerCameraShop.transform.position.x + count, playerCameraShop.transform.position.y, playerCameraShop.transform.position.z);
            }
            else
            {
                if (useInfiniteScrolling)
        ...
        public void GoToNextPanel(int count)
        {
            int nearestPanel = GetNearestPanel();
